...
func (e *engine) onRelayLatency(ev *events.CallRelayLatency) {
	...
	// Only endorse relays that are actually present in the offer. A caller may
	// probe its own nearest edge; echoing that relay back can make it elect a
	// relay for which this callee has no token/connection.
	e.mu.Lock()
	offered := map[string]bool{}
	if current := e.calls[ev.CallID]; current != nil && current.relay != nil {
		for i := range current.relay.endpoints {
			if name := current.relay.endpoints[i].relayName; name != "" {
				offered[name] = true
			}
		}
	}
	...
	for i := range rl.GetChildren() {
		...
		if len(offered) > 0 && !offered[name] {
			e.c.log.Debug().Str("call_id", ev.CallID).Str("relay_name", name).
				Msg("skipping relay-latency probe outside offered relays")
			continue
		}
...
// getAllMediaRelayEndpoints returns all relay endpoints from the relay data.
// For inbound direct 1:1 calls, connecting to all endpoints handles relay migration
// where the caller switches relays after accept.
func getAllMediaRelayEndpoints(rd *relayData) []*relayEndpoint {
	if rd == nil {
		return nil
	}
	eps := make([]*relayEndpoint, 0, len(rd.endpoints))
	for i := range rd.endpoints {
		eps = append(eps, &rd.endpoints[i])
	}
	return eps
}
